...
public class GameController {
    ...
    /** @return true اگر دستور مربوط به داخل بازی بود و پردازش شد */
    ...
    private boolean handleCheat(GameSession session, List<String> t, CommandLine cmd) {
        ...
        if (t.size() >= 2 && t.get(1).equals("spawn-zombie")) {
            ...
            String loc = cmd.get("l");
            // فرمت مورد انتظار: -l x,y  (پیاده‌سازی ساده بدون پارز پرانتز/کاما پیچیده)
            view.printMessage("زامبی " + type + " در مکان مشخص‌شده اضافه شد (نمونه‌ی ساده‌شده).");
            Zombie z = ZombieFactory.create(type);
            z.spawn(0, Board.COLS - 1);
    ...
    /** فرمت ورودی مورد انتظار برای مکان: "-l (x, y)" که ممکن است به چند توکن تقسیم شده باشد. */
