...
public final class ChapterPlan {
    ...
    // === رفع باگ: مقدار قبلی برای مرحله ۲ («۱۰ زامبی در ۵ ثانیه») مستقیماً
    // از عدد مثالِ سند («مثلاً ۱۰ زامبی را در ۵ ثانیه بکش») کپی شده بود، بدون
    // توجه به این‌که آن عدد فقط برای توضیح مفهوم «هدف + مهلت زمانی» در سند
    // آورده شده، نه یک مقدار قابل‌بازی. با سرعت اسپاون واقعی موج‌ها، در ۵
    // ثانیه‌ی اول اصلاً ۱۰ زامبی وارد زمین نمی‌شوند، پس این مرحله برای هر
    // بازیکنی از ابتدا غیرقابل‌برد بود. عدد زیر (۴۵ ثانیه برای ۱۰ زامبی) با
    // سرعت اسپاون موج معمولی هم‌خوان و واقعاً قابل‌برد است، در حالی که همچنان
    // فشار زمانی واقعی روی بازیکن می‌گذارد.
    public static int timedWarSecondsFor(int level) {
        return (level == 3) ? 60 : 45;
    }
